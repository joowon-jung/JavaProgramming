...
public class FinallyTest{
	...
	public void fileRead(String str){
		...
		// } catch(IOException e1) { => 해도 오류가 나지 않음 IOException 는 FileNotFoundException 을 포함하고 있기 때문에
			System.out.println("e1 : "+e1);
			System.out.println(str+" : File이 없습니다.");
		}catch(IOException e2){ // 읽다가 파일 깨졌을때
		// } catch(FileNotFoundException e1) { => 하면 오류남!  FileNotFoundException 는  IOException 의 하위클래스 이기 때문에
			System.out.println("e2 : "+e2);
			System.out.println("read() method에서 Exception 발생");
		}catch(Exception e3){ // 한꺼번에 CATCH 여러개 쓸 수 있음
